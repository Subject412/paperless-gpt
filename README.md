...
| `LLM_REQUEST_RATE`                  | Maximum request rate or minimum interval between requests for the main LLM (e.g. `500ms`, `10s`, `30m`, `1h`, `1d`, `2/h`, `30/m`). Plain numbers are treated as seconds.                    | No       | 500ms                      |
| `LLM_REQUESTS_PER_MINUTE`           | *(Deprecated: use LLM_REQUEST_RATE instead)* Maximum requests per minute for the main LLM.                                                                                                    | No       | 120                        |
| `LLM_MAX_RETRIES`                   | Maximum retry attempts for failed main LLM requests.                                                                                                                                          | No       | 3                          |
| `LLM_BACKOFF_MAX_WAIT`              | Maximum wait time between retries for the main LLM (e.g., `30s`, `10m`, `1d`).                                                                                                                | No       | 30s                        |
...
| `VISION_LLM_REQUEST_RATE`           | Maximum request rate or minimum interval between requests for the Vision LLM (e.g. `500ms`, `10s`, `30m`, `1h`, `1d`, `2/h`, `30/m`). Plain numbers are treated as seconds.                 | No       | 500ms                      |
| `VISION_LLM_REQUESTS_PER_MINUTE`    | *(Deprecated: use VISION_LLM_REQUEST_RATE instead)* Maximum requests per minute for the Vision LLM.                                                                                         | No       | 120                        |
| `VISION_LLM_MAX_RETRIES`            | Maximum retry attempts for failed Vision LLM requests.                                                                                                                                        | No       | 3                          |
| `VISION_LLM_BACKOFF_MAX_WAIT`       | Maximum wait time between retries for the Vision LLM (e.g., `30s`, `10m`, `1d`).                                                                                                             | No       | 30s                        |
...
```yaml
environment:
  VISION_LLM_REQUEST_RATE: "30m"      # Send at most 1 request every 30 minutes (or e.g. "2/h", "500ms", "1d")
